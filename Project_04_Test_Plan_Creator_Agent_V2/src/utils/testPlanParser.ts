...
const HEADER_ALIASES: Record<keyof ParsedTestCase | 'jiraKey', RegExp> = {
  ...
  name: /^(name|title|test[\s-_]?case|scenario|description)$/i,
...
export const parseTestPlanMarkdown = (markdown: string): ParsedTestCase[] => {
  ...
  // If the table has no recognizable headers, fall back to positional guesses
  if (colIndex.name === undefined) {
    colIndex.tcId = 0;
    colIndex.name = 1;
    if (headerCells.length >= 4) colIndex.steps = 2;
    if (headerCells.length >= 5) colIndex.expectedResult = 3;
    if (headerCells.length >= 6) colIndex.priority = 4;
  }
