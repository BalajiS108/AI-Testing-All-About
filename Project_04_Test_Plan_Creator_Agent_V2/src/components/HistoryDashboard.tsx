...
const Sparkline: React.FC<{ points: { x: string; y: number }[] }> = ({ points }) => {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-20">
      <line x1={padding} x2={width - padding} y1={height - padding} y2={height - padding} stroke="currentColor" strokeWidth="0.5" className="text-slate-200 dark:text-slate-700" />
      ...
      {points.map((p, i) => {
        ...
        return (
          <g key={i}>
            <circle cx={x} cy={y} r="3" className="text-blue-500" fill="currentColor" />
            <title>{p.x}: {p.y}%</title>
          </g>
        );
      })}
