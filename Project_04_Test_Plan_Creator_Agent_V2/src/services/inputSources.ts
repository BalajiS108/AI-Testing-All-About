...
export const fetchFromHtml = async (input: { url?: string; html?: string }): Promise<HtmlResult> => {
  try {
    const res = await axios.post(`${backendUrl()}/api/input/html`, input, { timeout: 30000 });
    return { source: 'html', label: res.data.label, items: res.data.items || [] };
  } catch (e: any) {
    unwrapError(e, 'HTML');
    throw e;
  }
